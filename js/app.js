...
function initSchaefflerChart() {
    ...
    // Define boundary lines data points (Approximate)
    // Line 1: Austenite / Martensite boundary
    const amLine = [
        { x: 0, y: 18 }, { x: 8, y: 10 }, { x: 16, y: 0 }
    ];

    // Line 2: Martensite / Ferrite boundary
    const mfLine = [
        { x: 0, y: 0 }, { x: 14, y: 0 }, { x: 28, y: 0 } // Represents axis, logic handles boundaries better via regions
    ];

    // Simplified boundaries for visual reference (Line segments)
    // A+M | Martensite
    const boundaryAM = [
        { x: 0, y: 18 }, { x: 17, y: 0 }
    ];
    ...
    // M+F | Ferrite
    // const boundaryMF = [ ... ]; // Complex curves

    // Since accurate curves are hard to draw with simple lines, we will rely on plotting the point
    // and potentially using a background image in a future iteration. 
    // For now, we plot the user's point on a blank grid with axis labels.
    ...
    const data = {
        datasets: [
            ...
            // Boundaries (Approximate visual guides)
            {
                label: 'A/M 分界 (Ref)',
                data: boundaryAM,
                showLine: true,
        ...
        options: {
            ...
            scales: {
                x: {
                    ...
                    title: {
                        display: true,
                        text: '鉻當量 Cr_eq = Cr + Mo + 1.5Si + 0.5Nb'
                    },
                    min: 0,
                    max: 30
                },
            ...
            plugins: {
                ...
                annotation: {
                    // Future: Use chartjs-plugin-annotation for regions if available
                }
            }
        }
    });
